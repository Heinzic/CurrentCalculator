...
function AddConsumerModal({active, setActive, sections}: CreateObjectProps) {
    ...
    const {register, watch, reset} = useForm<IConsumerForm>({mode:'onBlur', })
               ...
               <form action="" onClick={(e) => e.stopPropagation()} className="w-[858px] bg-[#D0DADF]">
                    <div className="p-[15px]">
                        <div className="flex flex-col px-[38px] gap-[10px] pt-[41px] bg-[#FFFFFF] rounded-md text-black">
                            ...
                            <select {...register('type', {required:true})} className="py-[6px] px-[25px] flex-grow bg-[#EBEBEB] rounded-md text-xl">
                                {types.data?.map(e => (
                                    <option value={e.id}>{e.name}</option>
                                ))}
                            </select>
                            <select {...register('section', {required:true})} className="py-[6px] px-[25px] flex-grow bg-[#EBEBEB] rounded-md text-xl">
                                {sections.map(e => (
                                    <option value={e.id}>{e.name}</option>
                                ))}
                            </select>
